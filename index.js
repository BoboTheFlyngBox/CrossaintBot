...
client.on('interactionCreate', async (interaction) => {
  if(interaction.isButton()){
    try{
      ...
      if(action === 'confirm_reset'){
        await uPinMsg(interaction.channel, channel.competition);
  ...
  try{
    ...
    if(interaction.commandName === 'nm'){
      ...
      await interaction.reply(`Started the current competition for League ${leagueNumber}   Registration is now open. Time limit: ${fT(maxTimeLimitSeconds)}.`);
      return;
    }
    ...
    if(interaction.commandName === 'reg'){
      ...
      await interaction.reply(
        `Registered **${fPN(competition.registeredPlayers[interaction.user.id])}** for ${fCL(competition)} (${gRC(competition)} total).`,
      );
      return;
    }
    ...
    if(interaction.commandName === 'unreg'){
      ...
      await interaction.reply(`Unregistered **${fPN(registeredPlayer)}** from ${fCL(competition)}.`);
      return;
    }
